...
void AAS_Reachability_JumpPad(void) {
	int face2num, i, ret, area2num, ent;
	//int modelnum, ent2;
	...
	for (i = 0; i < area2->numfaces; i++) {
		...
		for (n = 0; n < 1/*2*/; n++) {
			...
			if (ret && speed < 300) {
				...
				// movement prediction
				AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qtrue, qfalse, velocity, cmdmove, 30, 30, 0.1f, SE_TOUCHTELEPORTER|SE_TOUCHJUMPPAD|SE_HITGROUNDAREA|SE_HITGROUNDDAMAGE|SE_ENTERLAVA|SE_ENTERSLIME|SE_GAP, area2num);
