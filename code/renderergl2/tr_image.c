...
void GL_TextureMode(const char *string) {
	...
	// HACK to prevent trilinear from being set on voodoo, because their driver freaks...
...
image_t *R_CreateImage2(const char *name, byte *pic, int width, int height, GLenum picFormat, int numMips, imgType_t type, imgFlags_t flags, int internalFormat) {
	...
	if (textureFilterAnisotropic && !cubemap) {
		qglTextureParameteriEXT(image->texnum, textureTarget, GL_TEXTURE_MAX_ANISOTROPY_EXT, mipmap ? (GLint)Com_Clamp(1, maxAnisotropy, r_ext_max_anisotropy->integer) : 1);
	}
