...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker) {
	...
	if (targ->client->pers.teamState.lasthurtcarrier && level.time - targ->client->pers.teamState.lasthurtcarrier < CTF_CARRIER_DANGER_PROTECT_TIMEOUT && !attacker->client->ps.powerups[flag_pw]) {
		...
		attacker->client->ps.eFlags &= ~(EF_AWARD_EXCELLENT|EF_AWARD_IMPRESSIVE|EF_AWARD_GAUNTLET|EF_AWARD_CAP|EF_AWARD_DEFEND|EF_AWARD_ASSIST);
...
int Team_TouchOurFlag(gentity_t *ent, gentity_t *other, int team) {
	...
	// add the sprite over the player's head
	other->client->ps.eFlags &= ~(EF_AWARD_EXCELLENT|EF_AWARD_IMPRESSIVE|EF_AWARD_GAUNTLET|EF_AWARD_CAP|EF_AWARD_DEFEND|EF_AWARD_ASSIST);
	...
	for (i = 0; i < g_maxclients.integer; i++) {
		...
		if (player->client->sess.sessionTeam != cl->sess.sessionTeam) {
			...
			if (player->client->pers.teamState.lastreturnedflag + CTF_RETURN_FLAG_ASSIST_TIMEOUT > level.time) {
				...
				// add the sprite over the player's head
				player->client->ps.eFlags &= ~(EF_AWARD_EXCELLENT|EF_AWARD_IMPRESSIVE|EF_AWARD_GAUNTLET|EF_AWARD_CAP|EF_AWARD_DEFEND|EF_AWARD_ASSIST);
