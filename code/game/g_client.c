...
void ClientUserinfoChanged(int clientNum) {
	...
	*/
	...
	if (ent->r.svFlags & SVF_BOT) {
		...
		s = va("n\\%s\\t\\%i\\tt\\%d\\tl\\%d\\model\\%s\\hmodel\\%s\\c1\\%s\\c2\\%s\\w\\%i\\l\\%i\\g_redteam\\%s\\g_blueteam\\%s",
			client->pers.netname, client->sess.sessionTeam, teamTask, teamLeader, model, headModel, c1, c2, client->sess.wins, client->sess.losses, redTeam, blueTeam);
	}
